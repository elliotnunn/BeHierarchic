...
func (sa *arsenicData) getCode(symhigh uint32, symlow uint32, symtot uint32) { /* aka remove symbol */
	...
	sa.bitbuf = FillBigEndian(sa.bitbuf, sa.br)
...
func (sa *arsenicData) writeAndUnrleAndUnrnd(byteremain uint32, block []byte, rnd int16) uint32 {
	...
	for _, ch := range block {
		...
		if count == 4 {
			for range ch {
				byteremain--
				err := sa.bw.WriteByte(last)
				if err != nil || byteremain == 0 {
					return 0
				}
			}
			count = 0
...
func arsenic(r io.Reader, dstsize uint32) io.ReadCloser {
	pr, pw := io.Pipe()
	go arseniccopy(pw, r, dstsize)
	return pr
}
