...
func cvt(path string, fromfmt Fmt, tofmt Fmt) {
	println(path)

	if fromfmt != Darwin || tofmt != Rez {
		panic("unimplemented format conversion")
	}

	rfork, _ := os.ReadFile(path + "/..namedfork/rsrc")
	// okay to ignore error, means resfork empty
	...
	var finfo [32]byte
	nread, _ := getxattr(path, "com.apple.FinderInfo", &finfo[0], 32, 0, 0)
	// again, ok to ignore error
	if nread != len(finfo) {
		copy([]byte("????????"), finfo[:])
	}
	...
	var hasidump bool
	var idump []byte
	if string(finfo[:8]) != "\x00\x00\x00\x00\x00\x00\x00\x00" && string(finfo[:8]) != "????????" {
		hasidump = true
		idump = finfo[:8]
	}

	if hasrdump {
		os.WriteFile(path+".rdump", rdump, 0o666)
	} else {
		os.Remove(path + ".rdump")
	}

	if hasidump {
		os.WriteFile(path+".idump", idump, 0o666)
	} else {
		os.Remove(path + ".idump")
	}

	// Finally clean up the darwin versions
	if len(rfork) != 0 {
		// Truncate doesn't seem to work
		os.WriteFile(path+"/..namedfork/rsrc", nil, 0o666)
	}

	removexattr(path, "com.apple.FinderInfo", 0)
}
