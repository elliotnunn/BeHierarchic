...
func deRez(fork []byte) []byte {
	...
	sort.Slice(resMap.list, func(i, j int) bool {
		a := &resMap.list[i]
		b := &resMap.list[j]

		// unsigned comparison of types
		if a.tType < b.tType {
			return true
		} else if a.tType > b.tType {
			return false
		}

		// signed comparison of IDs
		if a.rID < b.rID {
			return true
		} else if a.rID > b.rID {
			return false
		...
		panic("two resources of the same type!")
	})
...
func dumpMap(flat []byte) (deep mapStruct) {
	...
	for i := uint16(0); i < typeCount; i++ {
		...
		for j := uint16(0); j < tCount; j++ {
			...
			if rNameOff != 0xffff {
				pascal := flat[mNames+rNameOff:]
				pascal = pascal[1 : 1+pascal[0]]
				res.name = string(pascal)
				res.hasName = true
			}
